...
const app={};
{
    ...
    function clickHandler(tagInput,dataList,container){
        const inputValueList = tagInput.value.trim().split(/[\n，,、\s+]/);
        for(let item of inputValueList){
            if(item!==""){
                dataList.push(item);
            }
        }
    ...
    function tag3ClickHandler(){
        clickHandler(tag3.input,tag3.dataList,tag3.container);
    }
    app.init=function(){
        tag1.input.addEventListener("keyup",tag1KeyUpHandler);
        tag2.input.addEventListener("keyup",tag2KeyUpHandler);
        $("button").addEventListener("click",tag3ClickHandler);
    }
    Object.freeze(app);
}
